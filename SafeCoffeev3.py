...
QPushButton#RecoverBtn:hover { background-color: #006666; border: 1px solid #8A2BE2; }
"""
...
class SafeCoffee(QMainWindow):
    def __init__(self):
        ...
        self.recover_btn = QPushButton("RECOVER CONNECTION")
        self.recover_btn.setObjectName("RecoverBtn")
        self.recover_btn.clicked.connect(self.recover_network)
    ...
    def recover_network(self):
        iface = self.engine.interface
        self.feed.append("<font color='#008080'>[*] Attempting to re-engage shield...</font>")
        # Re-enable the hardware
        os.system(f"sudo ip link set {iface} up")
        # Restart Network Manager service for auto-reconnect
        os.system("sudo systemctl restart NetworkManager")
        
        self.status.setText("RECONNECTING...")
        self.status.setStyleSheet("color: #FFFFFF; background-color: #1B2436;")
        self.feed.append("<font color='#008080'>[+] Interface UP. Nudging Network Manager.</font>")
